...
public class FailureDetector implements IFailureDetector, FailureDetectorMBean
{
    ...
    public void interpret(InetAddress ep)
    {
        ...
        {     
        		//Util.debug("fd ::: phi = " + phi + "... phiConvictThreshold_ = " + phiConvictThreshold_);
        		//###################HACK
						//###################JINSU
						//We know that all nodes are alive and dead via other methods.
						//disable this for loop.
						if(true) {
            for ( IFailureDetectionEventListener listener : fdEvntListeners_ )
            {
            		//Util.debug("fd interpret(1) convicting " + ep + " in " + listener);
                listener.convict(ep);
            }
            }
