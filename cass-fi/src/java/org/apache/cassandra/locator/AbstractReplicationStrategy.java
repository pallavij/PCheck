...
public abstract class AbstractReplicationStrategy
{
    ...
    public Multimap<InetAddress, InetAddress> getHintedEndpoints(String table, Collection<InetAddress> targets)
    {
        ...
        Util.debug("all nodes alive num = " + map.size());
            return map;
        }
        	
        Util.debug("map size in ars : " + map.size() + " ?? target size in ars : " + targets.size());
        ...
        for (InetAddress ep : targets)
        {
            ...
            map.put(destination, ep);
				...
				Util.debug("map size after 2nd pass in ars : " + map.size());
        return map;
    }
