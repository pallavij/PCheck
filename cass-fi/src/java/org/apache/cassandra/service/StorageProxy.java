...
public class StorageProxy implements StorageProxyMBean
{
    ...
    */
                    ...
                    for (Map.Entry<InetAddress, Collection<InetAddress>> entry : hintedEndpoints.asMap().entrySet())
                    {
                        ...
                        if (targets.size() == 1 && targets.iterator().next().equals(destination))
                        {
                            ...
                            else
                            {
                                ...
                                // jinsu this is how the data messages are sent.
                                Util.debug("sp line 153 sendoneway");
                                MessagingService.instance.sendOneWay(unhintedMessage, destination);
                            }
                        }
                        else
                        {
                        		System.out.println("checkpoint0.5");
                            // hinted
                            ...
                            // jinsu this is how the data messages are sent.
                            Util.debug("sp line 172 sendoneway");
    ...
    public static void mutateBlocking(List<RowMutation> mutations, ConsistencyLevel consistency_level) throws UnavailableException, TimeoutException
    {
        ...
        try
        {
            for (RowMutation rm : mutations)
            {
                ...
                Util.debug(" * mutateBlocking begin insert :: Key => " + rm.key() + " with  Consistency => " + consistency_level);
                //Util.debug("nep size: " + naturalEndpoints.size());
               	Util.debug("nep = " + naturalEndpoints);
                ...
                //jinsu
                //Util.debug("wep size: " + writeEndpoints.size());
                Util.debug("wep = " + writeEndpoints);
                ...
                for (Map.Entry<InetAddress, Collection<InetAddress>> entry : hintedEndpoints.asMap().entrySet())
                {
                    ...
                    {
                        // unhinted writes
                        //Util.debug("- sp sending unhinted, =D");
                        if (destination.equals(FBUtilities.getLocalAddress()))
                        {
                        		//Util.debug("inserting local message");
                        		Util.debug(Util.getNodeIdFromINetAddr(FBUtilities.getLocalAddress()) + " sends to " 
                            					+ Util.getNodeIdFromINetAddr(destination) + " ::: Msg => key = " + rm.key());
                        ...
                        else
                        {
                            ...
                            // jinsu this is how the data messages are sent.
                            //Util.debug("sp line 261 unhinted sendoneway =D");
                            Util.debug(Util.getNodeIdFromINetAddr(FBUtilities.getLocalAddress()) + " sends to " 
                            					+ Util.getNodeIdFromINetAddr(destination) + " ::: Msg => key = " + rm.key());
                            MessagingService.instance.sendOneWay(unhintedMessage, destination);
                        }
                    ...
                    {
                    //jinsu
                    		//Util.debug("- sp sending hinted, =O");
                        ...
                        Util.debug("sp line 286 hinted sendoneway =D");
                        MessagingService.instance.sendOneWay(hintedMessage, destination);
                    }
                }
            }
            ...
            throws UnavailableException
    {
        ...
        Util.debug("--liveNodes = " + liveNodes);
