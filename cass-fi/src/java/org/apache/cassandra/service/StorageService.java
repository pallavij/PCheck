...
public class StorageService implements IEndPointStateChangeSubscriber, StorageServiceMBean
{
    ...
    */
    ...
    {
        //JINSU
        Util.debug("- SS oC(2) MOVE_STATE  ==> " + MOVE_STATE + " ?==? apStateName ==> " + apStateName + " from " + endpoint);
				
        if (!MOVE_STATE.equals(apStateName)) {
        		Util.debug("- inside SS oC(2) MOVE_STATE  ==> " + MOVE_STATE + " ?==? apStateName ==> " + apStateName + " from " + endpoint);
        	    return;
        }
        ...
        String moveValue = apStateValue.substring(index+1);
				...
				//JINSU
				Util.debug("- SS oC(2) moveName = " + moveName + " ... moveValue = " + moveValue);
    ...
    private void handleStateLeaving(InetAddress endPoint, String moveValue)
    {
        ...
        // normally
        Util.debug("hsL : tokenMetadata_.addLeavingEndPoint()");
    ...
    private void handleStateLeft(InetAddress endPoint, String moveValue)
    {
        ...
        if (typeOfState.equals(LEFT_NORMALLY))
        {
            ...
            if (tokenMetadata_.isMember(endPoint))
            {
                ...
                Util.debug("hsLeft : tokenMetadata_.remoteEndPoint()");
                tokenMetadata_.removeEndpoint(endPoint);
            }
        }
        else
        ...
        Util.debug("hsLeft : tokenMetadata_.removeBootstrapToken()");
    ...
    private void removeEndPointLocally(InetAddress endPoint)
    {
        ...
        Util.debug("rEPL : tokenMetadata_.removeEndpoint()");
        tokenMetadata_.removeEndpoint(endPoint);
    }
