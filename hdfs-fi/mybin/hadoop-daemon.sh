...
# ------------------------------------------------------------------
# haryadi: overwrite logfile and pid files, and add datanode id
# ------------------------------------------------------------------
if [ "$command" = "datanode" ]; then

    DATANODE_ID=`cat $HADOOP_CONF_DIR/datanode-id`

    if [ ! -e $HADOOP_CONF_DIR/datanode-id ]; then
	echo ""; echo ""; echo ""; echo ""; echo ""; echo ""; 
	echo "---------------------------------------------";
	echo "ERROR: NO DATANODE-ID FILE !!!!!!!!!!!!"
	echo ""; echo ""; echo ""; echo ""; echo ""; echo ""; 
    else
	echo ""; echo ""; echo ""; 
	echo "------------------------------------------"
	echo "  ./hadoop-daemon.sh DataNode-$DATANODE_ID"
	echo "------------------------------------------"
	echo ""; echo ""; echo ""; 
    fi

    export HADOOP_LOGFILE=hadoop-$HADOOP_IDENT_STRING-p$command-$HOSTNAME-$DATANODE_ID.log
    export _HADOOP_DAEMON_PIDFILE=$HADOOP_PID_DIR/hadoop-$HADOOP_IDENT_STRING-p$command-$DATANODE_ID.pid
export _HADOOP_DAEMON_OUT=$HADOOP_LOG_DIR/hadoop-$HADOOP_IDENT_STRING-p$command-$HOSTNAME-$DATANODE_ID.out
fi
# ------------------------------------------------------------------
