...
public abstract class Server {
  ...
  private class Listener extends Thread {
	...
	// **************************************************
	// haryadi: let's print something for context creation
	// we should see incoming connectin either from the client
	// or the datanodes
	// **************************************************
  ...
  private int channelWrite(WritableByteChannel channel, 
    ...
    // **************************************************
    // haryadi: this is where we write to the channel ..
    //          there is no flush ... 
    // **************************************************
    int count;
    if (buffer.remaining() <= NIO_BUFFER_LIMIT) {
      // FIUtil.debug(">> S.channelWrite (1)");
      count = channel.write(buffer);
    } else {
      // FIUtil.debug(">> S.channelWrite (2)");
      count = channelIO(null, channel, buffer);
    }
    // **************************************************
    // original lines:
  ...
  private int channelRead(ReadableByteChannel channel, 
    ...
    // **************************************************
    // haryadi: this is where we read from the channel 
    // **************************************************
    int count;
    if (buffer.remaining() <= NIO_BUFFER_LIMIT) {
      // FIUtil.debug(">> S.channelRead (1) ");
      count = channel.read(buffer);
    } else {
      // FIUtil.debug(">> S.channelRead (2) ");
      count = channelIO(channel, null, buffer);
    }
    // **************************************************
    // original lines
    // int count = (buffer.remaining() <= NIO_BUFFER_LIMIT) ?
