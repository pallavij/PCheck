...
abstract class SocketIOWithTimeout {
  ...
  static void connect(SocketChannel channel, 
    ...
    try { 
      ...
      // *******************************************************
      // haryadi, bug fix, ClosedByInterruptException
      // the problem with channel.connect is that it can throw
      // ClosedByInterruptException if it's trying to connect, but this
      // thread is being interrupted ... Read sun java docs regarding
      // this exception.  I think it's the semantic of ClosedByInterruptException
      // that needs to be propertly dealt with.
      // but the problem is this exception can be thrown from many places.
      // the one that I care is from channel.connect(endpoint) inside
      // NetUtils.connect().
      // so what i'm doing is to convert closedbyinterruptException
      // to a socketitmeoutexception so that the higher layer 
      // (e.g. ipc/Client.java could retry (see Client setupConnection)
      // *******************************************************
      /**************************************
      try {
        ...
        FIUtil.debug(">> SIOWT.connect, ClosedByInterruptException, bug fix, ep = " + endpoint);
        throw new SocketTimeoutException("haryadi, ClosedByInterruptException hack");
      }
      **************************************/
      // *******************************************************
      // original lines:
