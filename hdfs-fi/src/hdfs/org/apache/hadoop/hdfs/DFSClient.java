...
public class DFSClient implements FSConstants, java.io.Closeable {
  ...
  // *******************************************************************
  // haryadi, thanh, hack, to get the block ID, pallavi-15
  // *******************************************************************
  public Block[] getBlocks(String src, long start,
                           long length) throws IOException {
    LocatedBlocks blocks = callGetBlockLocations(namenode, src, start, length);
    if (blocks == null) {
      return new Block[0];
    ...
    int nrBlocks = blocks.locatedBlockCount();
    Block[] blkList = new Block[nrBlocks];
    int idx = 0;
    for (LocatedBlock blk : blocks.getLocatedBlocks()) {
      assert idx < nrBlocks : "Incorrect index";
      blkList[idx] = blk.getBlock();
      idx++;
    }
    return blkList;
  }
  ...
  class LeaseChecker implements Runnable {
    ...
    synchronized void put(String src, OutputStream out) {
	  ...
	  // ********************************************
          // haryadi, hack, let renewLease finish first in daemon.start, bad hack, pallavi-13
          // ********************************************
          try {
            FIUtil.debug(">> DFSC.LC.put, rpc, nn.renewLease, hacked one");
    ...
    private void renew() throws IOException {
      ...
      // ********************************************
      // haryadi, hack : never run repeated renew lease, bad hack, pallavi-13
      // ********************************************
      FIUtil.debug(">> DFSC.LC.renew, should run, rpc, nn.renewLease, but disabled");
      // namenode.renewLease(clientName);
      // ********************************************
  ...
  ****************************************************************/
  class DFSOutputStream extends FSOutputSummer implements Syncable {
	...
	//PALLAVI: adding an upper bound to the no of retries
	int nRetries = 0;
	int maxRetries = 20;
        while (!fileComplete) {
		...
		if(nRetries > maxRetries){
			//PALLAVI:throw an exception
		        IOException result = new IOException("PALLAVI:No of retries exceeded");
		        throw result;
			//break;
		}
              }
