...
class BlockReceiver implements java.io.Closeable, FSConstants {
  ...
  BlockReceiver(Block block, DataInputStream in, String inAddr,
	...
	// ******************************************************
        // haryadi, hack, bufferedoutputstream, pallavi-11
        // make this.out a buffered output stream
        // ******************************************************
        this.out = new BufferedOutputStream(streams.dataOut);
        // ******************************************************
        // original line:
  ...
  private void setBlockPosition(long offsetInBlock) throws IOException {
    ...
    // *****************************************************************
    // haryadi, hack, bufferedoutputstream, pallavi-11, 
    // make sure we flush 'out' before we 
    // get the channel position
    // we flush 'out', because out is non-buffered anymore
    // out is BufferedOutputStream, while streams.dataOut is file output stream
    // *****************************************************************
    out.flush();
    // *****************************************************************
    ...
    private synchronized void lastDataNodeRun() {
      ...
      while (running && datanode.shouldRun && !lastPacket) {
        ...
        try {
            ...
            HdfsFiUtil.commonReplyOutFlush(replyOut); //replyOut.flush();
        } catch (Exception e) {
