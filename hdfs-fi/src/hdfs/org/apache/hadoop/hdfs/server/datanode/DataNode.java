...
import org.apache.hadoop.hdfs.HdfsFiUtil;
...
public class DataNode extends Configured
  ...
  boolean fiDisallowDnaTransfer = false; // haryadi/pallavi, new stuff
  ...
  // *********************************************************
  // haryadi: rpc hack, pallavi05
  // *********************************************************
  public int getServerPort() throws IOException {
    if (selfAddr != null) return selfAddr.getPort();
    return 1000; // unknown
  }
  // *********************************************************
  ...
  void startDataNode(Configuration conf,
    ...
    // *******************************************
    // haryadi/pallavi: extra control configuration
    // *******************************************
    this.fiDisallowDnaTransfer = conf.getBoolean("fi.disallow.dna.transfer", false);
    // *******************************************
    ...
    if (conf.get("slave.host.name") != null) {
      ...
      FIUtil.debug(">> Dn.sdn, (1) mName = " + machineName);
    ...
    if (machineName == null) {
      ...
      FIUtil.debug(">> Dn.sdn, (2) mName = " + machineName);
    ...
    if ( reason == null ) {
      //pallavi, haryadi : do not want to run this scanner  
  ...
  private void register() throws IOException {
    ...
    while(shouldRun) {
      try {
        ...
        FIUtil.debug(">> DN.register (0) name : " + dnRegistration.name);
        FIUtil.debug(">> DN.register (1) before : " + dnRegistration);


        dnRegistration = namenode.register(dnRegistration);

        FIUtil.debug(">> DN.register (2) after : " + dnRegistration);
  ...
  // ************************************************************************
  // haryadi, pallavi: new code, for block report
  // the following two functions are used in offerServive for stable
  // state checking
  // ************************************************************************
  private boolean isCmdNullOrFinalize(DatanodeCommand cmd){
    if(cmd == null){
      return true;
    }
    int action = cmd.getAction();
    if(action == DatanodeProtocol.DNA_FINALIZE){
      FIUtil.debug(">> DN.icnof, return true, receive finaliaze");
      return true;
    }
    FIUtil.debug(">> DN.icnof, returning false, cmd is: " + action);
    return false;
  }

  // ************************************************************************
  private boolean areAllCmdsNullOrFinalize(DatanodeCommand[] cmds){
    if(cmds == null)
      return true;
    for(DatanodeCommand cmd : cmds){
      if(!isCmdNullOrFinalize(cmd))
        return false;
    }
    return true;
  }
  ...
  // ************************************************************************ END
   ...
   //pallavi, haryadi : hack, send one heartbeat after reboot
   static int myHeartBeatCount = 0;
   static int maxHeartBeatCount = 1;
  ...
  public void offerService() throws Exception {
    ...
    while (shouldRun) {
      try {
        ...
        // ******************************************************************
        // haryadi, pallavi, hack: make sure, push block report to faster rate
        // ******************************************************************
        int dnId = selfAddr.getPort() - 50010;
        if (!FIUtil.isExperimentRunning() &&
            HdfsFiUtil.isBlockReportFlagExist(dnId)) {


          FIUtil.debug(">> DN.os, force, sendHeartBeat !!!!!! ");
          ...
          FIUtil.debug(">> DN.os, force, sendBlockReport !!!!!! ");
	  myHeartBeatCount++;
          ...
          if(isCmdNullOrFinalize(cmd) && areAllCmdsNullOrFinalize(cmds)) {
            FIUtil.debug(">> DN.os, no commands, delete flag file ");
            HdfsFiUtil.removeBlockReportFlag(dnId);
          }
          else{
            FIUtil.debug(">> DN.os, process some commands ");
            processCommand(cmds);
            processCommand(cmd);
          }
        }
        ...
        if (startTime - lastHeartbeat > heartBeatInterval) {
          ...
          // ******************************************************************
          // haryadi, pallavi, hack: don't interfere running experiment
          // ******************************************************************
          DatanodeCommand[] cmds = null;
          
	  //pallavi, haryadi : don't send heartbeat if myHeartBeatCount is >= 1, that is,
	  //if you already have sent a heartbeat
	  //we just want a datanode to send only one heartbeat after it reboots
	 
	  if (myHeartBeatCount >= maxHeartBeatCount) {
	  //if(FIUtil.isExperimentRunning()){
            FIUtil.debug(">> DN.os, disable original send hearbeat ");
            cmds = null;
          }
          else{

            FIUtil.debug(">> DN.os, sending heartbeat original ");
            cmds = namenode.sendHeartbeat(dnRegistration,
                                          data.getCapacity(),
                                          data.getDfsUsed(),
                                          data.getRemaining(),
                                          xmitsInProgress.get(),
	  ...
	  namenode.blockReceived(dnRegistration, blockArray, delHintArray);
        ...
        // ********************************************************************
        // haryadi, pallavi, hack:
        // we've sent the block report above so we're gonna disable
        // the logic below
        // ********************************************************************
        if (false) {
          // ********************************************************************
  ...
  private boolean processCommand(DatanodeCommand cmd) throws IOException {
    ...
    case DatanodeProtocol.DNA_TRANSFER:
      ...
      // **************************************************
      // haryadi, pallavi: hack, disable transferBlocks DNA_TRANSFER !!
      // careful !! not interfere
      // **************************************************
      if (fiDisallowDnaTransfer) {
        FIUtil.debug(">> Dn.pc, disallow DNA_TRANSFER ...");
      }
      else {
        FIUtil.debug(">> DN.pc received DNA_TRANSFER, and transferring ...");
    ...
    case DatanodeProtocol.DNA_INVALIDATE:
      ...
      //
      FIUtil.debug(">> Dn.pc, DNA_INVALIDATE ...");
    ...
    case DatanodeProtocol.DNA_SHUTDOWN:
      FIUtil.debug(">> Dn.pc, DNA_SHUTDOWN ...");
      // shut down the data node
      this.shutdown();
      return false;
    case DatanodeProtocol.DNA_REGISTER:
      // namenode requested a registration - at start or if NN lost contact
      FIUtil.debug(">> Dn.pc, DNA_REGISTER ...");
    ...
    case DatanodeProtocol.DNA_FINALIZE:
      FIUtil.debug(">> Dn.pc, DNA_FINALIZE ...");
    ...
    case DatanodeProtocol.DNA_RECOVERBLOCK:
      FIUtil.debug(">> Dn.pc, DNA_RECOVERBLOCK ...");
      // ****************************************************
      // haryadi, bad hack, pallavi-13, disable background recover blocks
      // ****************************************************
      FIUtil.debug(">> DN.pc, should recover Blocks ... but disabled !!");
  ...
  private LocatedBlock syncBlock(Block block, List<BlockRecord> syncList,
    ...
    if (!successList.isEmpty()) {
      ...
      namenode.commitBlockSynchronization(block,
                                          ...
                                          nlist);
      //haryadi, pallavi : sorting nlist according to port numbers to remove non-determinism here
      nlist = sortNodes(nlist);     
  ...
  //pallavi, haryadi: hacking to make the order of nodes in the pipeline deterministic 
  //Assumption here is that the ip is the same and the ports are what are different
  //So, we sort according to the ports
  private static DatanodeID[] sortNodes(DatanodeID[] list){
    int len = list.length;
    int[] ports = new int[len];
    String host = getHost(list);
    for(int i = 0; i < len; i++){
      int port = getPort(list, i);
      ports[i] = port;
    }
    Arrays.sort(ports);

    DatanodeID[] newIds = new DatanodeID[len];
    for(int i = 0; i < len; i++){
      newIds[i] = getDnIdWPort(list, ports[i]);
    }
    return newIds;
  }

  private static DatanodeID getDnIdWPort(DatanodeID[] list, int port){
    int len = list.length;
    for(int i = 0; i < len; i++){
      int p = getPort(list, i);
      if(p == port){
	return list[i];
      }
    }
    //pallavi: we should not be reaching here
    return null;
  }

  private static int getPort(DatanodeID[] list, int index){
    DatanodeID dnID = list[index];
    String ip = dnID.getName();
    String[] ipParts = ip.split(":");
    int port = Integer.parseInt(ipParts[1]);
    return port;
  }

  private static String getHost(DatanodeID[] list){
    DatanodeID dnID = list[0];
    String ip = dnID.getName();
    String[] ipParts = ip.split(":");
    return ipParts[1];
  }
