...
public class DatanodeDescriptor extends DatanodeInfo {
  ...
  void addBlocksToBeInvalidated(List<Block> blocklist) {
	...
	FIUtil.debug("DD.abtbi, add to invalidateBlocks, blk = " + blk);
  ...
  BlockCommand getInvalidateBlocks(int maxblocks) {
    ...
    // ****************************************
    if (deleteList == null) {
      FIUtil.debug("DD.gib, no delete list, no invalidate");
      return null;
    }
    else {
      FIUtil.debug("DD.gib, new cmd DNA_INVALIDATE");
      BlockCommand bc = new BlockCommand(DatanodeProtocol.DNA_INVALIDATE, deleteList);
      return bc;
    }
    // ****************************************8
