...
public class FSNamesystem implements FSConstants, FSNamesystemMBean {
  ...
  public synchronized void registerDatanode(DatanodeRegistration nodeReg
    ...
    // *************************************
    // haryadi, hack: keep datanode ip alias, otherwise
    // if we call Server.getRemoteAddress(), it will change the alias
    // to localhost 127.0.0.1
    String dnAddress = nodeReg.getHost();
    // *************************************
    // original line:
  ...
  private void resolveNetworkLocation (DatanodeDescriptor node) {
    ...
    // *****************************************************
    // haryadi: hack, rack aware script, since I don't know how to write
    // the script, we hack it here
    // *****************************************************
    networkLocation =
      HdfsFiUtil.getRackNetLocation(networkLocation, node.getHostName(), node.getPort());
    // *****************************************************
  ...
  // #######################################################################
  // ####                                                               ####
  // ####              H E A R T B E A T    M O N I T O R               ####
  // ####                                                               ####
  // #######################################################################
  // #######################################################################
  ...
  // #######################################################################
  // ####                                                               ####
  // ####            R E P L I C A T I O N    M O N I T O R             ####
  // ####                                                               ####
  // #######################################################################
  // #######################################################################
    ...
    public void run() {
      while (fsRunning) {
        try {
          ...
          // *************************************************************
          // haryadi, pallavi, hack: replication monitor, don't interrupt
          // and only do so, if the workload driver allow
          // *************************************************************
          if (!FIUtil.isExperimentRunning() &&
              HdfsFiUtil.isRepMonitorFlagExist()) {

            FIUtil.debug(">> RM.run, replication monitor is running");
            computeDatanodeWork();
            processPendingReplications();

            HdfsFiUtil.removeRepMonitorFlag();

          } else {
            FIUtil.debug(">> RM.run, skip replication monitor");
          }
          // *************************************************************
          // original lines:
          // computeDatanodeWork();
  ...
  public int computeDatanodeWork() throws IOException {
    ...
    synchronized(heartbeats) {
      ...
      // *****************************************************
      // haryadi: hack, careful, compute datanode work,
      // it's possible nodesToProcess is small
      // *****************************************************
  ...
  void finalizeUpgrade() throws IOException {
    ...
    FIUtil.debug(">> FSNS.finalizeUpgrade() ... ");

    checkSuperuserPrivilege();

    FIUtil.debug(">> FSNS.finalizeUpgrade(), calling FSI.finalizeUpgrade  ");
  ...
  synchronized long nextGenerationStampForBlock(Block block) throws IOException {
    ...
    // haryadi, bug?, we still get into this problem !!!!!!!!
