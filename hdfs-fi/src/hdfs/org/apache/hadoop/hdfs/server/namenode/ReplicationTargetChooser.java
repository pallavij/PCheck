...
class ReplicationTargetChooser {
  ...
  private DatanodeDescriptor[] chooseRandom(int numOfReplicas, 
    ...
    while(numOfReplicas > 0) {
      ...
      // ***********************************************************
      // haryadi, hack, add deterministic nodes, remove randomness (Place 2)
      // if these nodes are not sequential, then keep trying
      // until certain retries
      // ***********************************************************
      if (!HdfsFiUtil.areChosenNodesSequential
	  (clusterMap, choosenNode.toString(), nodes, excludedNodes)) {
	continue;
      }
      // ***********************************************************
  ...
  private boolean isGoodTarget(DatanodeDescriptor node,
    ...
    // ********************************************
    // haryadi, hack, pallavi-16, always return true for isGoodTarget
    // ********************************************
    if (true) return true;
