...
aspect precedenceAspect {
  ...
  //PALLAVI: bcastHooks might be needed for the client write protocol
  //bcastHooks, contextWrapper, contextPassingNonDirect, contextPassing, contextOrigin;
  zkHooks, contextWrapper, contextPassingNonDirect, contextPassing, contextOrigin;
 ...
 Object around(SocketChannel sc, SocketAddress sa) : (call (boolean SocketChannel.connect(SocketAddress)) && target(sc) && args(sa) && !within(org.fi.*)) {
    ...
    if (localPort != 0) {
      if (CD.debug) {
        System.out.format("# Adding socket history (a) ... [%d] %s \n",
                          s.getLocalPort(), Util.getZkServerId());
      }
      Util.addZkSocketHistory(s);
    }
  ...
  Object around(SocketChannel sc, SocketAddress sa) : (call (boolean SocketChannel.connect(SocketAddress)) && target(sc) && args(sa) && !within(org.fi.*)) {
    if (sa instanceof InetSocketAddress) {
      ...
      int serverId = Util.getZkNodeFromPort(target_port);
      String c = "NetIO-Node-"+serverId+":"+target_port;
      Context ctx = new Context(c);
      ctx.setPort(target_port);
      sc.setContext(ctx);

      if (CD.debug) {
		FMJoinPoint fjp = new FMJoinPoint (thisJoinPoint);
 		System.out.format("# Context creation SC (%s) (%d) [%s][%d] \n", sc.context.getTargetIO(), sc.context.getPort(), fjp.getShortName(), fjp.getLine());
      }
    }
