...
public class Learner {       
    ...
    throws IOException, ConnectException, InterruptedException {
        ...
        //sock.setSoTimeout(2500000);
	//PALLAVI: increasing the number of tries to 10 from 5
	int nTries = 10;
        for (int tries = 0; tries < nTries; tries++) {
            try {
                sock.connect(addr, self.tickTime * self.syncLimit);
                //sock.connect(addr, 2500000);
	    ...
	    //PALLAVI: increasing the sleep time
	    LOG.info("Trying to connect to leader...nTries = " + (tries + 1));
            //Thread.sleep(1000);
            Thread.sleep(50000);
        }
