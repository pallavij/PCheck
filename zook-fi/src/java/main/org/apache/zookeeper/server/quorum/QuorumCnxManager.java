...
public class QuorumCnxManager {
	...
	//following getters added by PALLAVI
    ...
    //PALLAVI: Thread that executes loopback
    class Loopback extends Thread{
	...
	public void run(){
		while(true){
			try{
				...
				LOG.debug("Exception in Loopback thread");			
			}
		}
	}
	...
	public void put(Message m){
		try{
			...
			LOG.debug("Exception while adding message in the Loopback thread");			
		}
	}
    }
    ...
    public void toSend(Long sid, ByteBuffer b) {
	    ...
	    ********/
	    //PALLAVI: sending loopback message in a different thread
	    b.position(0);
	    Message m = new Message(b.duplicate(), sid);
	    lb.put(m);
        /*
    ...
    class SendWorker extends Thread {
		...
		/*****    
            	// Jason:
                MyUtil.debug(">>>>> JASON <<<<< QuorumCnxManager ln550 send: right before sending message!");
            	MyUtil.debug(">>>>> JASON <<<<< QuorumCnxManager ln550 send: remote addr="+
            			self.getQuorumAddress().getAddress().toString()+", local port="+
            			self.getQuorumAddress().getPort()+", target addr/port?="+
            			channel.socket().toString()+", target sid="+
            			sid);
		*****/		
    ...
    class RecvWorker extends Thread {
        ...
        public void run() {
            try {
                ...
                while (running && !shutdown && channel != null) {
                    ...
                    if (length > 0) {
                        ...
                        // Jason:
                        //MyUtil.debug(">>>>> JASON <<<<< QuorumCnxManager ln667 receive. before reading message!");
                        ...
                        message.position(0);
                        //PALLAVI: commenting synchronized out because each thread is going to wait before executing the put, and 
			//only one thread would be woken up at a time
			//synchronized (recvQueue) {
                        ...
                        // Jason:
			/*****
                        MyUtil.debug(">>>>> JASON <<<<< QuorumCnxManager ln667 receive. finish reading message!");
                    	MyUtil.debug(">>>>> JASON <<<<< QuorumCnxManager ln667 receive: local addr="+
                    			self.getQuorumAddress().getAddress().toString()+", local port="+
                    			self.getQuorumAddress().getPort()+", target addr/port?="+
                    			channel.socket().toString()+", target sid?="+
                    			sid);
